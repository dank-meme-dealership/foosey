...
def badges(league_id, player_id)
  ...
  # tournament hack
  badges[9] << badge('🏆', 'Thanksgiving 2018 Champs')   #peter
  badges[19] << badge('🏆', 'Thanksgiving 2018 Champs')  #justin
  badges[12] << badge('🥈', 'Thanksgiving 2018 2nd Place')   #adam
  badges[453] << badge('🥈', 'Thanksgiving 2018 2nd Place')  #james
  badges[1] << badge('🥉', 'Thanksgiving 2018 3rd Place')   #matt
  badges[483] << badge('🥉', 'Thanksgiving 2018 3rd Place')  #mark
...
def recalc_elo(timestamp, league_id)
  database do |db|
    ...
    # temporary array of hashes to keep track of player rankings
    elos = {}
    ladder = {}
    active = {}
    ...
    player_ids(league_id).each do |player_id|
      ...
      active[player_id] = db.get_first_value('SELECT Active FROM Player WHERE PlayerID = :player_id', player_id)
    ...
    db.execute('SELECT DISTINCT GameID
      ...
      when 2
        ...
        # ladder - only applies to active players
        if active[game[0]['PlayerID']] == 1 && active[game[1]['PlayerID']] == 1
