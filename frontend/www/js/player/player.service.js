(function () {
  ...
  function PlayerService(localStorage, FooseyService) {
    ...
    /**
     * Update the players and return one type of them:
     * all, active, inactive; default all
     * @param type an argument to return the type of players
     */
    function updatePlayers(type) {
      return FooseyService.getAllPlayers().then(
        function (players) {
          ...
          // active players
          service.active = _.filter(service.all, function (player) {
            return player.active
          });
          localStorage.setObject('activePlayers', service.active);

          // inactive players
          service.inactive = _.filter(service.all, function (player) {
            return !player.active
          });
